...
void Graph::mstKruskal() const {
	...
	int totWeight = 0;
	int counter = 0;
	std::vector<Edge> V; // Version 2

	Edge e; // Version 2
	V.push_back(e); // Version 2
	//Heap<Edge> H; // Version 1
	...
	for (int i = 1; i <= size; ++i) {
		...
		while (itr != table[i].end()) {
			if (i < itr->tail) {
				...
				//H.insert(*itr); // Version 1
				V.push_back(*itr); // Version 2
			}
			itr++;
		}
	}

	// Heap with all the edges
	Heap<Edge> H{ V }; // Version 2
